...
/**
 * bioRxiv Search via Valyu API
 * Full-text search across bioRxiv with semantic search capabilities
 */
...
/**
 * Search bioRxiv via Valyu API
 */
async function searchbioRxiv(query, maxResults = 10) {
...
(async () => {
  ...
  if (command === 'setup') {
    ...
    if (!query) {
      ...
      result = await searchbioRxiv(query, maxResults);
    }
  }

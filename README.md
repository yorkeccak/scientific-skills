...
# Find papers on specific genes
scripts/search "BRCA1 mutations breast cancer" 20

# Drug mechanisms
scripts/search "metformin mechanism of action diabetes" 10
```
...
```bash
# AI research
scripts/search "transformer attention mechanisms" 20

# Physics research
scripts/search "quantum entanglement experiments" 15

# Mathematics
scripts/search "prime number distribution" 10
```

### ChEMBL Search

```bash
# Find bioactive compounds
scripts/search "kinase inhibitors for cancer" 25

# Drug targets
scripts/search "EGFR targeting molecules" 20

# Compound properties
scripts/search "blood brain barrier penetration compounds" 15
```
...
scripts/search "phase 3 trials Alzheimer's disease" 20

# By intervention
scripts/search "CAR-T cell therapy trials" 15

# By status
scripts/search "recruiting trials breast cancer immunotherapy" 25
```

### Literature Search (Aggregated)

```bash
# Comprehensive search across all literature
scripts/search "mechanisms of cellular aging" 50

# Find all relevant research
scripts/search "CRISPR off-target effects" 40
```

### Biomedical Search (Aggregated)

```bash
# Clinical + research literature
scripts/search "long COVID treatment approaches" 30

# Disease mechanisms + trials
scripts/search "Parkinson's disease biomarkers clinical trials" 40
```

### Drug Discovery Search (Aggregated)

```bash
# Complete drug information
scripts/search "EGFR inhibitors side effects targets" 30

# Target validation
scripts/search "novel targets for rheumatoid arthritis" 25
```
